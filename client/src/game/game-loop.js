...
export class GameLoop {
    ...
    getInitialFrame() {
        ...
        while (nextBeat < this.song.beats.length && this.song.beats[nextBeat].timestamp < RENDER_TIME_WINDOW) {
            ...
            for (const note of beat.notes) {
                ...
                if (note.holdTo !== 0) {
                    note.holdWidthPercentage = 100 * (note.holdTo - beat.timestamp) / TIME_WINDOW;
                }
    ...
    start_without_adjustments() {
        ...
        this.gameLoop = setInterval(() => {
            ...
            while (this.nextBeat < this.song.beats.length && this.song.beats[this.nextBeat].timestamp < currentTimestamp + RENDER_TIME_WINDOW) {
    ...
    start_with_adjustments() {
        ...
        this.gameLoop = setInterval(() => {
            ...
            // Add new notes that are within the time window
            while (nextBeat < this.song.beats.length && this.song.beats[nextBeat].timestamp < currentTimestamp + RENDER_TIME_WINDOW) {
                let beat = this.song.beats[nextBeat];
                currentBeats.push(beat);
                nextBeat++;
                changed = true;
    ...
    start_with_fractions() {
        ...
        this.gameLoop = setInterval(() => {
            ...
            // Add new notes
            while (nextBeat < this.song.beats.length && currentFractionWindow < RENDER_MEASURES_PER_FRAME) {
                let beat = this.song.beats[nextBeat];
                currentBeats.push(beat);
                currentFractionWindow += beat.fraction;
                nextBeat++;
