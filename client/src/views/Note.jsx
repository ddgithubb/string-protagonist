...
export function Note({ initial, fret, duration, width, pause, pitch }) {
    ...
    return (
        <motion.div className={"base-note " + (isHold ? "base-note-long" : "")} style={{ left: left_transform, width }} exit={{ opacity: 0 }} transition={{ opacity: { duration: 0.2 } }}>
            <div className={isHold ? "base-note-fret-long" : ""} style={{
                backgroundColor: fillForProbability(keyProbabilities[pitch%12]),
            }}>
                {fret}
            </div>
        </motion.div>
