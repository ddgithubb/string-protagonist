...
export async function getSongData(songId, revisionId, image, trackNumber) {
    ...
    songData.tuning = measuresJSON.tuning;

    if (measureTimestampsJSON.length === 0) {
        return undefined;
    }

    const videoTimestamps = measureTimestampsJSON[measureTimestampsJSON.length - 1];

    songData.videoURL = "https://www.youtube.com/watch?v=" + videoTimestamps.videoId;
    ...
    songData.beats = measuresJSON.measures.flatMap((measure, index) => {
        ...
        currentSignatureFactor = measure.signature === undefined ? currentSignatureFactor : measure.signature[0] / measure.signature[1];
